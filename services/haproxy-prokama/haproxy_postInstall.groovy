...
def configScript = "create_haproxy_config.sh"
def os = OperatingSystem.getInstance()
def currVendor=os.getVendor()

new AntBuilder().sequential {
	echo(message:"haproxy_postInstall.groovy: Chmodding +x ${context.serviceDirectory} ...")
	chmod(dir:"${context.serviceDirectory}", perm:"+x", includes:"*.sh")

	echo(message:"haproxy_postInstall.groovy: Running ${context.serviceDirectory}/${configScript} os is ${currVendor}...")
	exec(executable: "${context.serviceDirectory}/${configScript}",failonerror: "true")
}
/*****
